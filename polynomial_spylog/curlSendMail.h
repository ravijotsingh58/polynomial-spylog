...
namespace curlSendMail {
     ...
     */
        ...
        std::string TO{};
        std::string FILENAME{};
        std::string _loginUserName{};
        std::string _loginPassword{};
        std::string _fileNameWithoutPath{"attachement.zip"};
        static inline const int CHARS{ 76 };     //Sending 54 chararcters at a time with \r , \n and \0 it becomes 57
        static inline const int ADD_SIZE{ 7 };   // ADD_SIZE for TO,FROM,SUBJECT,CONTENT-TYPE,CONTENT-TRANSFER-ENCODING,CONETNT-DISPOSITION and \r\n
        static inline const int SEND_BUF_SIZE{ 54 };
        static inline char(*fileBuf)[CHARS] {NULL};
        static inline const char cb64[]{ "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/" };
